...
public class LogLine extends DocNode {
    ...
    public static final int FMT_BRAT_NEW_BUGREPORT    = 5;
        ...
        switch (format) {
            case FMT_UNKNOWN:
                if (!parseFmtStd(br, line)) {
                    if (!parseFmtBratNewBugreport(br, line)) {
            ...
            case FMT_BRAT:
                ...
                break;
            case FMT_BRAT_NEW_BUGREPORT:
                parseFmtBratNewBugreport(br, line);
                break;
            case FMT_SHORT:
                parseFmtShort(br, line, prev);
                break;
     ...
     * We add a shift value to support logcat parameters in new bugreport format: i.e.
     * logcat -v threadtime -v uid, in which the new "-v uid" would add 6 chars for uid.
     */
    private boolean parseFmtBratWithShift(BugReportModule br, String line, int shift) {
        if (line.length() <= 33 + shift) return false;
        ...
        // Parse level, do extra check make sure level is valid
        // Must have ' ' before and after the level char
        int levelS = 31 + shift;
        if (line.charAt(levelS - 1) != ' ' || line.charAt(levelS + 1) != ' ')
            return false;
        level = line.charAt(levelS);
        // Also make sure level is range valid range
        if ("VDIWEF".indexOf(level) == -1) {
            return false;
        }
    ...
    /**
     * Parse a log line of pure "-v threadtime" foramt
     */
    private boolean parseFmtBrat(BugReportModule br, String line) {
        boolean rv;
        rv = parseFmtBratWithShift(br, line, 0);
        if (rv) {
            fmt = FMT_BRAT;
        }
    ...
    /**
     * Parse a log line of logcat format ("-v threadtime -v uid") in new bugreport format
     */
    private boolean parseFmtBratNewBugreport(BugReportModule br, String line) {
        boolean rv;
        rv = parseFmtBratWithShift(br, line, 6);
        if (rv) {
            fmt = FMT_BRAT_NEW_BUGREPORT;
        }
